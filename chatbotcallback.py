...
class chatbot(telepot.helper.ChatHandler):
	...
	def on_callback_query(self,msg):
		...
		items = self.db.get_items(self._chat_id)		
		if query_data in items:
			self.db.delete_item(query_data,self._chat_id)
			self.sender.sendMessage('Thanks for buying: ' + query_data)

		elif:
			sender.answerCallbackQuery(query_id, text='you already bought that', show_alert=True)
